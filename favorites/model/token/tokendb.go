...
func GetTokenByTimeRange(dateTime time.Time) (Tokenobjs []StructToken) {
	opendatabase()
	...
	for iter.Next() {
		...
		var newdata StructToken
		json.Unmarshal(value, &newdata)
		if newdata.TokenTime.After(dateTime) || dateTime.Equal(newdata.TokenTime) {
			Tokenobjs = append(Tokenobjs, newdata)
		}
	}
