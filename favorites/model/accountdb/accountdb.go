...
type AccountStruct struct {
	...
	TokenListArr               []TokenList //token list contains all token id specifies block index
	Filelist                   []FileList
	FavoredNodes               map[string]string
	Storage                    int
}
...
//TokenList token list contains token id and return block index made transaction by token id
...
func AccountUpdateUsingTmp(data AccountStruct) bool {
	...
	if data.SessionID != AccountStructObj.SessionID {

		AccountSessionStructObj := AccountSessionStruct{SessionId: data.SessionID, AccountIndex: data.AccountIndex}
		DeleteSession(AccountStructObj.SessionID)
		err := SessionCreate(AccountSessionStructObj)
		if !err {
			errorpk.AddError("AccountSessionStructObjCreate  AccountSessionStructObj package", "can't create AccountSessionStructObj", "runtime error")
			return false
		}

	}
	AccountLastUpdatedTimestructObj := AccountLastUpdatedTimestruct{AccountLastUpdatedTime: data.AccountLastUpdatedTime, AccountIndex: data.AccountIndex}
