...
func Mining() {
	for {
		...
		if len(transaction.GetPendingTransactions()) != 0 && transaction.CheckReadyTransaction() {
			...
			winnerObj.Index = globalPkg.RSAPublic + "_" + winnerObj.NextBlockIndex + "_" + errorpk.ConvertTimeToString(winnerObj.TimeStamp)
			...
			if winnerObj.WinnerValidator.ValidatorIP == validator.CurrentValidator.ValidatorIP {
				...
				broadcastTcp.SendObject(winnerObj, winnerObj.WinnerValidator.ECCPublicKey, "", "proofOfStake", validator.CurrentValidator.ValidatorSoketIP)
			} else {
				broadcastTcp.SendObject(winnerObj, winnerObj.WinnerValidator.ECCPublicKey, "", "proofOfStake", winnerObj.WinnerValidator.ValidatorSoketIP)
			}
...
func ForgeTheBlock(WinningValidatorObj WinningValidatorStruct) {
	if len(transaction.GetPendingTransactions()) != 0 {
		...
		if NextBlockIndex != globalPkg.ConvertFixedLengthStringtoInt(WinningValidatorObj.NextBlockIndex) {
			...
			WinnerCount = append(WinnerCount, WinningValidatorObj.CurrentNode.EncECCPublic)
		} else {
			WinnerCount = append(WinnerCount, WinningValidatorObj.CurrentNode.EncECCPublic)
		}
		...
		if len(WinnerCount) >= totalNumberOfValidators { ////should be vaild lenght of
			...
			blockObj.ValidatorIP = validator.CurrentValidator.ValidatorIP
			calculateBlockHash(&blockObj)
			///
			// selectTransactions(&blockObj)
			// blockObj.BlockTimeStamp = globalPkg.UTCtime()
			// blockObj.ValidatorIP = validator.CurrentValidator.ValidatorIP
			// // *blockObj.ECCPublicKey.X = *validator.CurrentValidator.ECCPublicKey.X
			// *blockObj.ECCPublicKey.Y = *validator.CurrentValidator.ECCPublicKey.Y
			// blockObj.ECCPublicKey.Curve = validator.CurrentValidator.ECCPublicKey.Curve
			// fmt.Println("proof blockObj.ECCPublicKey.X", validator.CurrentValidator.ECCPublicKey.X)
			// fmt.Println("proof blockObj.ECCPublicKey.Y", validator.CurrentValidator.ECCPublicKey.Y)
			// fmt.Println("proof blockObj.ECCPublicKey.Curve", validator.CurrentValidator.ECCPublicKey.Curve)
			// fmt.Println("*proof blockObj.ECCPublicKey.X", *validator.CurrentValidator.ECCPublicKey.X)
			// fmt.Println("*proof blockObj.ECCPublicKey.Y", *validator.CurrentValidator.ECCPublicKey.Y)
			// fmt.Println("proof blockObj.ECCPublicKey.Curve", validator.CurrentValidator.ECCPublicKey.Curve)

			// fmt.Println("blockObj.ECCPublicKey :                  ", blockObj.ECCPublicKey)
			// fmt.Println("validator.CurrentValidator.ECCPublicKe.Curve : ", validator.CurrentValidator.ECCPublicKey.Curve)
		(&blockObj).CalculateBlockHash()
...
// BoardcastHandleProofStak handle stake of stake
func BoardcastHandleProofStak(tCPDataObj broadcastTcp.TCPData) {
	var proofOSObjec WinningValidatorStruct
