...
func GetAdminByTimeRange(dateTime time.Time) (Admins []AdminStruct) {
	opendatabase()
	...
	for iter.Next() {
		...
		var newdata AdminStruct
		json.Unmarshal(value, &newdata)
		if newdata.AdminLastUpdateTime.After(dateTime) || dateTime.Equal(newdata.AdminLastUpdateTime) {
			Admins = append(Admins, newdata)
		}
	}
