...
//***************************************************************************************************************
func (TransactionObj TransactionDB) AddTransactiondb() bool {
	...
	if len(TransactionObj.TransactionInput) >= 1 {
		TransactionObj.TransactionKey = TransactionObj.TransactionInput[0].SenderPublicKey + "_" + TransactionObj.TransactionTimeUser.String() + "_" + TransactionObj.TransactionID
		haveSender = true
		...
		if firstAcc.AccountPublicKey == TransactionObj.TransactionOutPut[0].RecieverPublicKey {
			TransactionObj.TransactionKey = "0000" + "_" + TransactionObj.TransactionTimeUser.String() + "_" + TransactionObj.TransactionID
		}
	} else {
	...
	diff := TransactionObj.TransactionTimeUser.Sub(lastTx.TransactionTimeUser)
	if diff.Seconds() < 5 {
...
func GettransactionByID(transactionid string) (values TransactionDB) {
	opendatabase()
	iter := DB.NewIterator(nil, nil)
	for iter.Next() {

		value := iter.Value()
		var newdata TransactionDB
		json.Unmarshal(value, &newdata)
		if newdata.TransactionID == transactionid {
			values = newdata
		}
	}
	closedatabase()
