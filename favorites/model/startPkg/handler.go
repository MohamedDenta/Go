...
func HandleRequest() {
	...
	myRouter.Handle("/baf6bedb564d569489560e8e9fcbc686", globalPkg.IsAdmine(account.GetAllpkAPI)).Methods("POST", "OPTIONS")
	//token
	...
	myRouter.Handle("/8c31b2b83db823edfae89fb", globalPkg.IsUser(tokenModule.RegisteringBillingTokenAPI)).Methods("POST", "OPTIONS")
	...
	myRouter.Handle("/63f1b356c26945aa19ea798c", globalPkg.IsUser(tokenModule.GettokennameAPI)).Methods("POST", "OPTIONS")
	//account module
	...
	myRouter.Handle("/658d41bc1951eca24a99b", globalPkg.IsAdmine(account.BillingAccountRegisterAPI)).Methods("POST", "OPTIONS")
	...
	// billing
	myRouter.Handle("/9d0d42649bdd21a87069fb1", globalPkg.IsAdmine(transactionModule.TransferOwnershipAPI)).Methods("POST", "OPTIONS")
	myRouter.Handle("/ffba558c70fabd801204ed5", globalPkg.IsAdmine(transactionModule.GetAllOwnershipForBillingAccountAPI)).Methods("POST", "OPTIONS")
	myRouter.Handle("/8956068b9e5827a6733236c", globalPkg.IsAdmine(transactionModule.GetUserOwnerAPI)).Methods("POST", "OPTIONS")
	myRouter.Handle("/dca34482c40364d42", globalPkg.IsAdmine(transactionModule.CreateOwnershipAPI)).Methods("POST", "OPTIONS")
	myRouter.Handle("/6b3e5832rs14v1d8c0cb479bd0", globalPkg.IsAdmine(transaction.GetAllsignatureAPI)).Methods("POST", "OPTIONS")
	// myRouter.Handle("/cd86989fe3555840109f328eace2a63", globalPkg.IsAdmine(transaction.GetAllsignatureAPI)).Methods("POST", "OPTIONS")
	//Block
	...
	myRouter.HandleFunc("/GetPartialLegderAPI", ledger.GetPartialLegderAPI).Methods("POST", "OPTIONS")
	...
	//Dashboard
	myRouter.HandleFunc("/faa902663bcf4b3cbae01ae", dashboard.GetBlockDashboard).Methods("POST")
	...
	myRouter.Handle("/dy535dbf09ef2dafds87d019", globalPkg.IsAdmine(adminModule.GetValidatorListForAdmin)).Methods("POST", "OPTIONS")
	...
	//broadcastTCP
	myRouter.Handle("/83fe6e91e70f5fe1cee78jkw4", globalPkg.IsAdmine(broadcastTcp.GetAllsendObjectAPI)).Methods("POST", "OPTIONS")
	...
	myRouter.Handle("/abb3e16efadcbbe03e716473c", globalPkg.IsUser(file.AddFavouriteNode)).Methods("POST", "OPTIONS")
	myRouter.Handle("/cdaecdeabbe03e716473c", globalPkg.IsUser(file.RemoveFavouriteNode)).Methods("POST", "OPTIONS")
	myRouter.Handle("/acdeaeffddaa256523273c", globalPkg.IsUser(file.ExploreFavouriteNodes)).Methods("POST", "OPTIONS")

	myRouter.Handle("/ab3e140dawa3e716473c", globalPkg.IsAdmine(tokenModule.GetAllResponsesAPI)).Methods("POST", "OPTIONS")
