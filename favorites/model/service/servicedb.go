...
func (data *ServiceStruct) ServiceCreateOUpdate() bool {
	Opendatabase()
...
// GetCreateTime ...
func GetCreateTime(voucherId string) int64 {
	Opendatabase()
	iter := DB.NewIterator(nil, nil)
	for iter.Next() {
		...
		var newdata ServiceStruct
		json.Unmarshal(value, &newdata)
		if newdata.VoutcherId == voucherId {
			return newdata.CreateTime
		}
	}
	return -1
}
func GetserviceByTimeRange(dateTime time.Time) (purchasedobj []ServiceStruct) {
	Opendatabase()
	...
	for iter.Next() {
		...
		var newdata ServiceStruct
		json.Unmarshal(value, &newdata)
		if newdata.Time.After(dateTime) || dateTime.Equal(newdata.Time) {
			purchasedobj = append(purchasedobj, newdata)
		}
	}
