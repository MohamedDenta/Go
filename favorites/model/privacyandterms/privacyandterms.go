...
//getstr get id,userpk,user password 
type getstr struct {
	ID       string
	UserPK   string
	UserPass string
...
func GetByIDAPI(w http.ResponseWriter, req *http.Request) {
	...
	if accdata.AccountIndex != "" && accdata.AccountPassword == id.UserPass {
		...
		if PrivacyandtermsObj.ID == 0 && len(PrivacyandtermsObj.Items) == 0 {
			responseObj := responses.FindResponseByID("50")
		...
		responseObj := responses.FindResponseByID("51")
		globalPkg.SendError(w, responseObj.EngResponse)
		globalPkg.WriteLog(logobj, responseObj.EngResponse, "failed")
		return
	}
...
//UpdateAPI response with update privacyandterms
